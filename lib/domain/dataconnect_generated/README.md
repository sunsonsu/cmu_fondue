...
```dart
String userId = ...;
ConnectorConnector.instance.getUserById(
  userId: userId,
).execute();
```
...
final result = await ConnectorConnector.instance.getUserById(
  userId: userId,
);
...
final ref = ConnectorConnector.instance.getUserById(
  userId: userId,
).ref();
ref.execute();
...
String userId = ...;
String fcmToken = ...;
ConnectorConnector.instance.updateFcmToken(
  userId: userId,
  fcmToken: fcmToken,
).execute();
```
...
final result = await ConnectorConnector.instance.updateFcmToken(
  userId: userId,
  fcmToken: fcmToken,
);
...
final ref = ConnectorConnector.instance.updateFcmToken(
  userId: userId,
  fcmToken: fcmToken,
).ref();
ref.execute();
...
#### Return Type
`execute()` returns a `OperationResult<AddUpvoteData, AddUpvoteVariables>`
```dart
/// Result of an Operation Request (query/mutation).
class OperationResult<Data, Variables> {
  OperationResult(this.dataConnect, this.data, this.ref);
